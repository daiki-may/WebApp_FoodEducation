document.addEventListener("DOMContentLoaded", function () {
    ...
    let selectedExperiment = "VVP_Japanese"; // ✅ Ex1をデフォルト選択
    let preloadedVideos = {};
    let loadedVideos = 0;
    let totalVideos = 0;
    let currentPlayingVideo = null;

    // ✅ 初期状態で Ex1 ボタンを選択、他のボタンは無効化
    ...
    // 🎯 Experiment ボタンのクリックイベント（Ex1 以外はクリック不可）
    experimentButtons.forEach(button => {
    ...
    // 🎬 事前に Ex1 の動画 6 つをロードし、進捗を表示
    function preloadVideos() {
        const promises = [];
        let startTime = performance.now();

        fruitImages.forEach(fruit => {
            const videoUrl = fruit.getAttribute(`data-video-VVP_Japanese`); // ✅ Ex3 のみ
    ...
    // 🍓 フルーツ画像のクリックイベント（Ex1 以外は無効）
    fruitImages.forEach(fruit => {
        fruit.addEventListener("click", function () {
            if (selectedExperiment !== "VVP_Japanese") return; // 🚫 Ex1 以外ではクリック無効
